...
- **Issuer:** MLMWPSQNVAIBRFDHWCKSFOVUAZDDWKJGCLRSYZIUEFDURPWIPQXACYOEPMLB
- **Type:** External asset issued on QX 
...
Each stream fires independently daily at 11:00 UTC UTC if:
- ≥6 days elapsed since last payout for that stream
- Accumulated revenue ≥ 1,000 QU

**Distribution Formula (per stream):**
```
holderPortion   = poolRevenue × 900 ÷ 1000  → token holders
shareholderPortion    = poolRevenue × 100 ÷ 1000  → shareholders + clan

WP Token Holder split (900‰):
  - holderReward = eligibleBalance × holderPortion / totalSnapshot
  - excludes: SELF (treasury), fundraising wallet, exchange wallets

Shareholder & Clan split (100‰):
  - shareholderReward = qrwaShares × shareholderPortion / totalShares
  - clanReward = clanWeightedTokens × shareholderPortion / totalClanWeight
```
...
clanReward[member] = (memberRank × 1000) × shareholderPortion / clanWeightedTotal
```
...
- Fixed pool: `WOLFPACK_STAKING_REWARD_PER_EPOCH = 1,923,076 WP` (~100M ÷ 52 epochs)
...
- Hardcoded at INITIALIZE to token issuer: `MLMWPSQNVAIBRFDHWCKSFOVUAZDDWKJGCLRSYZIUEFDURPWIPQXACYOEPMLB`
...
Full contract implementation available at:
- **GitHub:** https://github.com/MZoxx/core/tree/feature/2026-05-18-ggwp-contract
- **File:** `src/contracts/GGWP.h` (967 lines)
- **Registration:** `src/contract_core/contract_def.h` (index 28, epoch 217)
...
2. **Epoch N+1 (216):** If approved, code merged; IPO conducted; QUs collected and burned
