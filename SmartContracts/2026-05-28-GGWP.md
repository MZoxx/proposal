...
- **Contract Index:** 28
- **Construction Epoch:** 216 (IPO: Epoch 215)
...
```
qminePortion   = poolRevenue × 900 ÷ 1000  → token holders
qrwaPortion    = poolRevenue × 100 ÷ 1000  → shareholders + clan

QMINE split (900‰):
  - holderReward = eligibleBalance × qminePortion / totalSnapshot
  - excludes: SELF (treasury), fundraising wallet, exchange wallets

qRWA split (100‰):
  - shareholderReward = qrwaShares × qrwaPortion / totalShares
  - clanReward = clanWeightedTokens × qrwaPortion / totalClanWeight
```
...
- **Registration:** `src/contract_core/contract_def.h` (index 28, epoch 216)
...
3. **Epoch N+2 (216):** Contract constructed; INITIALIZE executes; ready for user calls
